...
namespace BookManageSystem
{
    public partial class frmBookSortManage : Form
    {
        ...
        private void dgvDataBindingAllInfo()
        {
            ...
            this.dgvAllSortInfo.Columns["bookSortID"].HeaderText = "客房类别编号";
            this.dgvAllSortInfo.Columns["bookSortRemark"].HeaderText = "客房类别说明";
        }
        ...
        private void tsbAddSort_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("客房类别编号与客房类别说明不能为空！", "操作提示");
                }
            }
            catch (Exception ex)
            {
        ...
        private void tsbDeleteSort_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("删除" + bookSortID.ToString() + "的客房编号会删除所有与此关联的客房信息,确定要删除吗？", "操作提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
